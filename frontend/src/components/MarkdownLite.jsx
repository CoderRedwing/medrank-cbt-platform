...
/* Lightweight markdown + inline-math renderer for AI-generated text.
   ...
   headings (#/##/###), **bold**, *italic*, `inline code`, numbered/bulleted
   lists, blockquotes (>), horizontal rules (---), and $...$ inline LaTeX
   (via KaTeX, same approach as the existing MathText component). Not a
   full markdown engine — deliberately small since this only needs to
   cover model output, not arbitrary user-authored markdown. */
...
// Renders **bold**, *italic*, `code` and $math$ within a single line of text into React nodes.
    ...
    mathParts.forEach((part, i) => {
        ...
        boldParts.forEach((b, j) => {
            ...
            codeParts.forEach((c, k) => {
                ...
                if (/^`[^`]+`$/.test(c)) {
                    nodes.push(
                        <code key={`${keyPrefix}-c${i}-${j}-${k}`} style={{
                ...
                italicParts.forEach((it, l) => {
                    if (!it) return;
                    if (/^\*[^*]+\*$/.test(it)) {
                        nodes.push(<em key={`${keyPrefix}-i${i}-${j}-${k}-${l}`}>{it.slice(1, -1)}</em>);
                    } else {
                        nodes.push(<span key={`${keyPrefix}-t${i}-${j}-${k}-${l}`}>{it}</span>);
                    }
                });
            });
        });
...
export default function MarkdownLite({ text, style, className }) {
    ...
    lines.forEach((rawLine) => {
        ...
        if (line === '') { flushList(); flushQuote(); flushPara(); return; }
